...
def check_port_in_use(port):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        if s.connect_ex(("127.0.0.1", port)) == 0:
            print(
                f"{INDENTATION}-> A porta {port} já está em uso. Assumindo que é o nosso próprio serviço."
            )
            return True
        else:
...
def open_firewall(port):
    ...
    subprocess.run(
        cmd_delete, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
    ...
    try:
        ...
        print(
            f"{INDENTATION}-> Aviso: Falha ao liberar o firewall. Tem certeza que executou como Administrador?"
        )
...
def create_superuser(python_exe, username="admin", password="admin"):
    ...
    if result.returncode == 0:
        ...
        print(
            f"{INDENTATION}-> RECOMENDAÇÃO: Troque esta senha no painel administrativo depois."
        )
    elif "already exists" in result.stderr or "já existe" in result.stderr:
        print(
            f"{INDENTATION}-> O usuário '{username}' já existe no banco de dados. Pulando criação."
        )
    else:
        print(
            f"{INDENTATION}-> Aviso: Não foi possível criar o administrador automaticamente."
        )
        print(
            f"{INDENTATION}-> Detalhe: {result.stderr.strip() or result.stdout.strip()}"
        )
...
def setup_nssm_service(python_exe, port):
    ...
    run_command(
        ...
        "Instalando serviço base",
    )
    run_command(
        f'"{nssm_path}" set {service_name} AppParameters "-m waitress --port={port} config.wsgi:application"',
        "Configurando o Waitress como servidor",
    )
    run_command(
        f'"{nssm_path}" set {service_name} AppDirectory "{project_dir}"',
        "Definindo diretório de trabalho",
    )
    run_command(
        f'"{nssm_path}" set {service_name} DisplayName "Servidor Django - Recepção"',
        "Configurando nome de exibição",
    )
    run_command(
        f'"{nssm_path}" set {service_name} Description "Backend do sistema de recepção rodando na porta {port}"',
...
def main():
    if not is_admin():
        print("=" * 60)
        print(
            "⚠️  ATENÇÃO: Este script precisa ser executado com privilégios de Administrador."
        )
    ...
    run_command(
        ...
        "Instalando dependências na venv",
    )
